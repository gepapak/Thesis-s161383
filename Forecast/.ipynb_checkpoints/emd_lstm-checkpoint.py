...
def emd_lstm_model(new_data, i, look_back, data_partition, cap):
    # Filter and prepare raw signal
    data1 = new_data.loc[new_data['Month'].isin(i)].reset_index(drop=True).dropna()
    raw_signal = data1['hydro'].values.reshape(-1, 1)

    # Decompose using EMD
    emd = EMD()
    IMFs = emd(raw_signal.flatten())  # input must be 1D
    imf_df = pd.DataFrame(IMFs).T  # Transpose to get each IMF as a column

    # Lists to accumulate component-wise predictions
    pred_test_list = []
    test_true_list = []
    ...
    for col in imf_df.columns:
        ...
        test_true_list.append(y_test_inv)

    # Combine all IMFs: additive reconstruction
    y_pred_final = np.sum(pred_test_list, axis=0)
    y_true_final = np.sum(test_true_list, axis=0)
