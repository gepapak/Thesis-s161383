...
def lstm_model(new_data, i, look_back, data_partition, cap, target):
    # Filter and prepare data
    data1 = new_data.loc[new_data['Month'].isin(i)].reset_index(drop=True).dropna()
    s = data1[target].values.reshape(-1, 1)  # Ensure 2D
    ...
    # Define LSTM model
    model = Sequential()
    model.add(LSTM(units=128, input_shape=(X_train_lstm.shape[1], X_train_lstm.shape[2])))
    model.add(Dense(1))
