...
import wandb  # ✅ Make sure wandb is imported for logging
...
class ForecastWrapperEnv(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # ✅ Action Explainability Logging
        wandb.log({
            "timestep": self.env.t,
            "action_wind": float(action[0]),
            "action_solar": float(action[1]),
            "action_hydro": float(action[2]),
            "action_battery": float(action[3]),
            "action_battery_use": float(action[4]),
            **{f"forecast_{k}": float(v) for k, v in self.forecast_cache.items()}
        })
    ...
    def _update_forecasts(self):
        ...
        for var, model in self.forecast_models.models.items():
            ...
            try:
                ...
                actual = self.env.data[var].iloc[t] if t < len(self.env.data) else 0.0
                wandb.log({
                    f"{var}_forecast": forecast,
                    f"{var}_actual": actual,
                    "timestep": t
                })
