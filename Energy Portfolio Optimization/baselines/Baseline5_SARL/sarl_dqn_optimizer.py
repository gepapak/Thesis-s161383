...
class ActionSpaceConverter:
    ...
    def __init__(self):
        # Current Tier1 environment action contract:
        # investor_0 Box(1), battery_operator_0 Discrete, risk_controller_0 Box(1),
        # meta_controller_0 Box(1). Battery conversion to a discrete index is
        # handled by the runner because it depends on the active environment.
        self.action_mappings = {
            'investor_0': slice(0, 1),
            'battery_operator_0': slice(1, 2),
            'risk_controller_0': slice(2, 3),
            'meta_controller_0': slice(3, 4),
        }

        self.action_bounds = {
            'investor_0': {'low': -1.0, 'high': 1.0},
            'battery_operator_0': {'low': -1.0, 'high': 1.0},
            'risk_controller_0': {'low': -1.0, 'high': 1.0},
            'meta_controller_0': {'low': -1.0, 'high': 1.0},
        }
